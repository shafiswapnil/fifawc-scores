...
- **v1.0.0 released** on 2026-06-16. CI/CD pipeline verified end-to-end.
- `docs/appcast.xml` auto-updated by CI on each tag push (commit to main → GitHub Pages).
- Old tag re-creation safety: CI deletes existing release before re-creating.
- **App Icon** still uses placeholder — create real assets before wider distribution.
